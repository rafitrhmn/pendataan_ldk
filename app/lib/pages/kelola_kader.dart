...
class KelolaKader extends StatefulWidget {
  const KelolaKader({Key? key}) : super(key: key);

  @override
  State<KelolaKader> createState() => _KelolaKaderState();
}

class _KelolaKaderState extends State<KelolaKader> {
  late final KaderBloc _bloc;
  final TextEditingController _searchController = TextEditingController();

  @override
  void initState() {
    super.initState();
    _bloc = KaderBloc()..add(FetchKaderisasi()); // fetch awal
  }
      ...
      body: Padding(
        ...
        child: BlocBuilder<KaderBloc, KaderState>(
          ...
          builder: (context, state) {
            if (state is KaderLoading) {
              return const Center(child: CircularProgressIndicator());
            } else if (state is KaderLoaded) {
              if (state.allCadres.isEmpty) {
                return const Center(child: Text("Belum ada kaderisasi"));
              }
          ...
          child: Material(
            ...
            child: InkWell(
              onTap: () {
                showDialog(
                  context: context,
                  builder: (context) => const AddKaderDialog(),
                );
              },
              borderRadius: BorderRadius.circular(8.0),
      ...
      children: [
        Expanded(
          child: TextField(
            ...
            onChanged: (value) {
              _bloc.add(SearchKader(value)); // ✅ langsung pakai _bloc
            },
        ...
        IconButton(
          onPressed: () {
            _bloc.add(const SortKader(true)); // ✅
          },
...
// }
 ...
 // body: BlocBuilder<KaderBloc, KaderState>(
      ...
      //             ),
      //           ),
      //           // List data kaderisasi
      //           Expanded(
      //             child: RefreshIndicator(
      //               onRefresh: () async {
      //                 _bloc.add(FetchKaderisasi());
      //               },
      //               child: ListView.separated(
